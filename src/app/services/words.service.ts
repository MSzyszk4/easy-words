...
import {BehaviorSubject, map, Observable, Subject, toArray} from "rxjs";
@Injectable({
  providedIn: 'root'
})
export class WordsService {
  ...
  getNouns(): Observable<WordType> {
    return this.nouns.asObservable().pipe(
      map(word => { word.correct = word.type === Type.NOUN; return word;})
    );
  }

  getVerbs(): Observable<WordType> {
    return this.verbs.asObservable().pipe(
      map(word => { word.correct = word.type === Type.VERB; return word;})
    );
  }
  ...
  // check() {
  //   this.nouns.map(
  //     word => (word.correct = word.type === Type.NOUN)
  //   )
  //
  //   this.verbs.map(
  //     word => (word.correct = word.type === Type.VERB)
  //   )
  // }
